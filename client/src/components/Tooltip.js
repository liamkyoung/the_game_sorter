...
class Tooltip extends Component {
  ...
  finalFormatting() {
    ...
    return (
      <div className="tooltip" style={this.state.styles}>
        ...
        <h1 style={{ color: this.calculateColor() }}>{this.ratingText}</h1>
        <b>
          <h2>Genres: </h2>
        </b>
        <h3>{this.genreText}</h3>
        <b>
          <h2>Platforms: </h2>
        </b>
        <h3>{this.platformsText}</h3>
      </div>
    );
  }
  componentDidMount() {
  ...
  calculateColor() {
    ...
    if (!this.props.rating) return "#f00";
    else return color;
  }

  formatState(rating, genres, platforms) {
    if (!rating || rating === 0) this.ratingText = "N/A";
    else this.ratingText = Math.floor(this.props.rating).toString();

    for (let i = 0; i < this.props.genres.length; i++) {
      if (i !== genres.length - 1)
        this.genreText += this.props.genres[i] + ", ";
      else this.genreText += this.props.genres[i];
    }

    for (let i = 0; i < this.props.platforms.length; i++) {
      if (i !== platforms.length - 1)
        this.platformsText += this.props.platforms[i] + ", ";
      else this.platformsText += this.props.platforms[i];
    }
  }
  render() {
